...
├─ .claude/
│  ├─ agents/              ← 20 סוכני Claude Code (ראה רוסטר)
...
|---|---|---|---|---|
| `playwright` | Microsoft (רשמי) | ❌ | פעיל | דפדפן אמיתי: ניווט, לחיצות, צילומי מסך, גריפה — competitor-scout, site-keeper, qa-verifier, marketing, content-seo, creative-studio |
| `supabase` | Supabase (רשמי) | ✅ read-only | ממתין לטוקן | data-analyst, supabase-architect, finance-billing |
| `stripe` | Stripe (רשמי) | ✅ restricted | ממתין לטוקן | finance-billing |
...
```
(נותן PDF/Word/Excel/PowerPoint — ל-content-seo, legal-privacy, finance-billing.)
...
- `cyber-guardian` בודק כל MCP חדש לפני הוספה. גרסאות — לשקול נעילה (במקום `@latest`).
