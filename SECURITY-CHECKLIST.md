...
### ✅ תוקן (2026-07-16) — `guardPublicMaintenance` שולח אדמין ל-`/maintenance`
היה: הגייט בדק רק `getSessionUser()` הישן (קוקי לקוח) — אדמין שמחובר **רק** לפאנל (לא גם
כלקוח) נשלח ל-`/maintenance` בדפים ציבוריים כמו מבקר רגיל. תוקן ב-`system-settings.ts`:
`guardPublicMaintenance()` בודק כעת קודם `getAdminSessionUser()` (קוקי `rb-admin-auth`
המבודד), ורק אם אין — נופל חזרה ל-`getSessionUser()`. בנוסף כוסו 5 נתיבים ציבוריים שלא
קראו לגייט כלל: `login`, `reset-password`, `cancel`, `preview`, `[slug]` (עמודי CMS).
